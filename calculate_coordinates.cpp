...
void calculate_angles_from_coordinates(){
    ...
    for (unsigned int i = 0; i < number_of_coordinate_blocks; i++){
        ...
        */
        ...
        zeta = sqrt(pow(deg(zeta)-180, 2)); // zeta now in degree and being transformed into the angle marked in the geogebra file and made positive
        mu = 180-deg(mu); // mu now in degree and being transformed into the angle marked in the geogebra file (adjacent angle)
